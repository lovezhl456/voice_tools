...
- 状态：实现与本地验收完成，待 PR 审阅；独立分支 `enhance/v0.17.2-config-editor`，基于最新主线 `679ee2c`。
- 范围：补齐图形化指标／参数／组合规则／输出标签编辑、导入导出、配置版本保存与本机运行；在原复核页增加明确入口，保留原试听和标注。
- 验证：Python 626 项（588 通过、38 原有条件跳过）、Node 24 项、CLI 9 场景通过；最终安装包原 QA 16 项与检测／编辑器18项桌面／手机检查通过。原专项脚本仍待后续迁移。见[界面说明](docs/detection-editor.md)与[验收记录](docs/detection-editor-validation.md)。
