...
- 当前已合入基线：**`0.8.1`**，对应 PR #14 合并提交 `d44599c`。
- 本工作分支源码、CLI 和机器接口文档为 **`0.10.1`**；ViSQOL 仍处于开发／待审阶段。
- 并行版本 **`0.9.1`** 保留给批量／SIPp；本次 ViSQOL 目标 **`0.10.1`**，合并前核对主线版本与交付顺序。
...
- **状态：已合入 main，PR #14，合并提交 `d44599c`。**
...
## 0.10.1 · ViSQOL 全参考音质评估

- 状态：开发中，尚未交付。
- 工作分支：`feat/v0.10.1-visqol`；`0.9.1` 保留给并行的批量/SIPp 迭代。
- 范围：ViSQOL 环境检查、单对/批量 CLI、固定版本安装脚本、官方样本实战、下载/安装/使用文档与结构化结果。
- 验收：待本机源码构建、真实后端评分、失败路径和回归检查完成后补充。
