...
- 范围：逐项审核 705 个 Python／Node 用例，最终删除 19 项（17 Python、2 Node）；独有失败模式均保留，清单标明入口／输入／分支／结果覆盖依据。
- 审查修复：修正误删的发现入口、任务文件输出／runtime 边界、抓包隔离、媒体证据、精确指标、缓存与依赖诊断；追加“公共契约不能仅因涉及常量而删除”和“混合用例保留独有断言”的规则。
- 验证：安装包浏览器 46 项、实际 CLI／任务流程 23 次调用、Studio 23 项及 CLI 兼容 9 场景通过；最终全量与可复现证据见 [测试精简记录](docs/test-pruning.md)。
