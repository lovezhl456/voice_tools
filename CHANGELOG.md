...
## 0.18.1 · E2E 优先与低价值测试删除（不变更产品版本）

- 状态：实现与本地验证完成，待 PR 审阅；分支 `chore/v0.18.1-e2e-test-pruning`，基于 `653288f`。仅修改测试与协作约定，沿用实际包版本 0.18.1。
- 范围：三个并行审查组逐项审核 705 个 Python／Node 用例，删除 53 项，记录保留项的具体故障与 E2E 缺口；新增 E2E 优先、产物复核和编码前失败分析规则。
- 验证：安装包浏览器 46 项、真实本机 SIP／时序 27 项通过；最终普通回归与逐项去留证据见 [测试精简记录](docs/test-pruning.md)。
