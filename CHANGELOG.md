...
- 状态：实现与本地验收完成，[PR #33](https://github.com/lovezhl456/voice_tools/pull/33) 待审阅，尚未合并；分支 `fix/v0.16.2-assessment-waveform`，基于已合并的 `08f861b`。
...
- 防遗漏措施：能力保留、关键拒绝条件和分层交付证据写入仓库约定及 PR 模板；合成夹具、安装包浏览器回归与 PR 自动检查纳入仓库，沿用 PR #33。默认 Python 583 项（545 通过、38 条件跳过）、Node 24 项、CLI 9 场景通过，桌面／手机 R01–R08 共 16 项通过；隐藏波形反例被正确拒绝。详见[落实验收记录](docs/review-acceptance-validation.md)。
