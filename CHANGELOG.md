...
- `0.9.1` 已由并行 SIP 批量队列迭代占用（PR #16），本次新功能使用 `0.10.1`，保留该迭代。
...
- 本地验证：Mac M1 Max / 32 GiB，官方权重真实推理成功；离线结果与 TorchMetrics 1.9.0 对齐。全仓 321 项测试，301 项执行通过、20 项可选测试跳过；基础 wheel 在未安装 Torch 的环境中完成命令发现与诊断。
...
- 本轮同步的主线基线为 **`0.8.1`**，对应 PR #14 合并提交 `d44599c`；`0.9.1` 由并行 PR #16 使用。
...
- 尚未验证生产网关／运营商线路、NAT；该轮不修改或发布 HTML。合入记录见本节开头。
