...
- 全仓沙箱运行：249 项，17 项可选原生测试跳过，2 组 setUpClass 因禁止绑定本机 HTTP 端口失败；这不代表全仓验收通过。
- 原生首轮：预期 486 测试通过，综合测试发现 RTP 统计缺失；已修正 SWIG 对象生命周期读取，最终原生复测待本机端口权限放行。新增静音 RTP、无 RTP、接收重复 DTMF、无应答等本机用例尚待运行。
