...
def contract(parser, selected=None, native_parsers=None):
    ...
    return {"latency_contract": public_contract(), "schema_version": "1.0", "tool_version": __version__, "selected_tool": selected,
            ...
            "output": {"detect": "SQLite library; immutable definitions/batches, automatic tags and independent review revisions; report is a snapshot, review-import persists browser drafts; config-ui edits offline definitions; serve emits status=listening then runs a loopback-only editor/save/run service", "gaps": "independent interval candidates, event exclusions and imported evidence; review-check validates separate human labels", "audio_qa": "one JSON object on stdout with ok, exit_code, status, summary, artifacts; errors have error.code/message",
