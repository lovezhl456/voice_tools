...
录音质检在本机 CPU 上离线运行；HOMER 查询连接你配置的 HOMER 7 服务，保存的 trace 可以离线分析。两者都不需要 GPU 或在线模型。
...
open outputs/demo/report.html  # macOS；其他系统用浏览器打开
```
...
| [工具源码](src/voice_tools/tools) | `recording_qa/` 与 `homer/`，各自维护业务逻辑 |
