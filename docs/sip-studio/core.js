...
(function (root) {
  ...
  const uid = () => 's' + (globalThis.crypto?.randomUUID?.() || Math.random().toString(36).slice(2));
  const definitions = {
    wait: {name:'等待', icon:'◷', color:'amber', hint:'给 IVR 留出播报时间', defaults:{seconds:2}},
    play: {name:'播放录音', icon:'▷', color:'blue', hint:'8 kHz · 单声道 PCM16', defaults:{file:'assets/question.wav'}},
    dtmf: {name:'发送按键', icon:'#', color:'violet', hint:'RFC 4733 / SIP INFO', defaults:{digits:'1#',method:'rfc4733',duration_ms:160,gap_ms:100}},
    play_media: {name:'回放 PCAP 媒体', icon:'≋', color:'teal', hint:'引用转换后的 media.json', defaults:{file:'media/media.json'}},
    hangup: {name:'挂断', icon:'↙', color:'rose', hint:'结束通话并收尾', defaults:{}}
  };
  const makeStep = action => ({id:uid(),label:definitions[action].name,action,...clone(definitions[action].defaults)});
  const defaultEnv = () => ({id:uid(),name:'本地回环',config:{target_uri:'sip:peer@127.0.0.1:5070',account:{id_uri:'sip:tester@127.0.0.1'},network:{sip_port:0,rtp_port:4000},codec:'PCMA',connect_timeout_s:30,max_call_s:120,record_early:false}});
  const makeCase = (envId, title='IVR 按键导航') => ({id:uid(),title,tags:'IVR, 回归',envId,updated:new Date().toISOString(),steps:[{...makeStep('wait'),label:'等待欢迎语',seconds:2},{...makeStep('dtmf'),label:'选择业务菜单',digits:'1#'},{...makeStep('wait'),label:'接收应答',seconds:3},makeStep('hangup')]});
  ...
  function str(v,label,max=256) {if(typeof v!=='string'||!v||v.length>max||/[\x00-\x1f]/.test(v)) throw Error(label+' 必须是有效文本');return v;}
  function num(v,low,high,label,integer=false){if(typeof v!=='number'||!Number.isFinite(v)||v<low||v>high||(integer&&!Number.isInteger(v)))throw Error(label+` 须为 ${low}–${high}${integer?' 的整数':''}`);return v;}
  function uri(v,label,server=false){
    str(v,label);
    const host='(?:[A-Za-z0-9.-]+|\\[[0-9A-Fa-f:]+\\])';
    const user=server?'':"[A-Za-z0-9_.!~*'()%+\\-]+@";
    if(!new RegExp('^sip:'+user+host+'(?::[0-9]{1,5})?(?:;transport=udp)?$').test(v))throw Error(label+' 仅支持 sip: 地址及 UDP');
  ...
  function validateScenario(data, assets=[]) {
    ...
    check(()=>{
      ...
      if('auth' in a){object(a.auth,['username','realm','password_env'],'auth');str(a.auth.username,'认证用户名');str(get(a.auth,'realm','*'),'realm');str(a.auth.password_env,'password_env');if(!/^[A-Za-z_][A-Za-z0-9_]*$/.test(a.auth.password_env))throw Error('password_env 只能填写环境变量名');}
    });
    check(()=>{
      const n=get(data,'network',{});object(n,['sip_port','rtp_port','bind_address','public_address'],'network');num(get(n,'sip_port',0),0,65535,'SIP 端口',true);num(get(n,'rtp_port',4000),1024,65000,'RTP 端口',true);if((get(n,'rtp_port',4000))%2)throw Error('RTP 端口必须为偶数');
      ['bind_address','public_address'].forEach(k=>{if(k in n && (typeof n[k]!=='string'||!/^\d{1,3}(\.\d{1,3}){3}$/.test(n[k])||n[k].split('.').some(v=>Number(v)>255||(v.length>1&&v[0]==='0'))))throw Error(k+' 必须为 IPv4 地址');});
    });
    check(()=>{if(!['PCMA','PCMU'].includes(get(data,'codec','PCMA')))throw Error('仅支持 PCMA / PCMU');num(get(data,'connect_timeout_s',30),1,120,'接通超时');num(get(data,'max_call_s',120),1,900,'最长通话');if('record_early' in data && typeof data.record_early!=='boolean')throw Error('record_early 必须为布尔值');});
    if(!Array.isArray(data.steps)||data.steps.length<1||data.steps.length>256){errors.push('用例须包含 1–256 个步骤');return {errors,warnings,knownDuration,unknownDuration};}
    data.steps.forEach((step,i)=>check(()=>{
      const label=`步骤 ${i+1}`;if(!step||typeof step.action!=='string'||!Object.hasOwn(stepFields,step.action))throw Error(label+' 不支持的 action');
      object(step,['action',...stepFields[step.action]],label);
      if(step.action==='wait')knownDuration+=num(step.seconds,0,900,label+' 等待时间');
      if(step.action==='dtmf'){
        str(step.digits,label+' 按键',128);if(!/^[0-9*#ABCD]+$/.test(step.digits))throw Error(label+' 按键只能使用 0–9、*、#、A–D');
        if(!['rfc4733','sip_info'].includes(get(step,'method','rfc4733')))throw Error(label+' 按键方式无效');
        knownDuration+=step.digits.length*(num(get(step,'duration_ms',160),40,2000,label+' 持续时间',true)+num(get(step,'gap_ms',100),40,2000,label+' 间隔',true))/1000;
      }
      if(['play','play_media'].includes(step.action)){
        str(step.file,label+' 素材路径',4096);
        const asset=assets.find(a=>a.path===step.file&&a.kind===(step.action==='play'?'wav':'media'));
        if(asset&&Number.isFinite(asset.duration)&&asset.duration>0)knownDuration+=asset.duration;else unknownDuration++;
        warnings.push(`${label}：执行前须由 CLI 校验 ${step.file} 的文件${step.action==='play_media'?'及音频哈希':''}。`);
      }
      if(step.action==='hangup'&&i!==data.steps.length-1)throw Error('挂断必须是最后一个步骤');
    }));
    if(knownDuration>(get(data,'max_call_s',120)))errors.push('已知动作时长超过最长通话时间');
    if(unknownDuration)warnings.push(`${unknownDuration} 个素材时长待确认，完整预算须由 CLI 校验。`);
  ...
  function wavInfo(buffer){
    const v=new DataView(buffer),read=(p,n)=>String.fromCharCode(...new Uint8Array(buffer,p,n));
    if(v.byteLength<44||read(0,4)!=='RIFF'||read(8,4)!=='WAVE'||v.getUint32(4,true)+8>v.byteLength)throw Error('不是完整的 RIFF WAV');
    let format=null,audio=null;const end=v.getUint32(4,true)+8;
    for(let p=12;p+8<=end;){const name=read(p,4),size=v.getUint32(p+4,true),start=p+8;if(start+size>end)throw Error('WAV 数据被截断');if(name==='fmt '&&size>=16)format={type:v.getUint16(start,true),channels:v.getUint16(start+2,true),rate:v.getUint32(start+4,true),align:v.getUint16(start+12,true),bits:v.getUint16(start+14,true)};if(name==='data')audio={start,size};p=start+size+(size%2);}
    if(!format||!audio||format.type!==1||format.channels!==1||format.rate!==8000||format.bits!==16||format.align!==2)throw Error('仅接受 8 kHz、单声道 PCM16 WAV');
    const duration=audio.size/16000;num(duration,0.001,900,'WAV 时长');if(audio.size%2)throw Error('WAV 样本不完整');
    const peaks=[];for(let i=0;i<96;i++){let peak=0;const start=Math.floor(i*audio.size/2/96),end=Math.floor((i+1)*audio.size/2/96);for(let n=start;n<end;n++)peak=Math.max(peak,Math.abs(v.getInt16(audio.start+n*2,true))/32768);peaks.push(peak);}
    return {duration,peaks};
  }
  function mediaInfo(data){
    object(data,['schema_version','audio','audio_sha256','duration_s','dtmf','source','warnings'],'media');
    if(data.schema_version!=='1.0')throw Error('media 版本须为 1.0');str(data.audio,'audio 路径',4096);if(!/^[a-f0-9]{64}$/.test(data.audio_sha256))throw Error('media 缺少有效 SHA256');num(data.duration_s,.001,900,'media 时长');
    if(!Array.isArray(get(data,'dtmf',[]))||(get(data,'dtmf',[])).length>256)throw Error('DTMF 事件列表无效');let end=-1;
