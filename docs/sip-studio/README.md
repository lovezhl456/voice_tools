...
- 批量队列：快照加入、排序、重复次数、并发 / 端口池、queue.json 下载和结果导入。
- 独立 SIPp 压力包：XML、CSV、DTMF PCAP 与启动脚本；详见 [安装与使用](../sipp.md)。
...
未接入 ASR、条件分支、在线插件安装或浏览器真实执行服务。批量与压力使用各自的导出文件和 CLI 命令，不污染单通话 scenario.json。
