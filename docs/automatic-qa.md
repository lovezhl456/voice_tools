...
支持 8–48 kHz、单／双声道 PCM16 WAV，单批最多 200 个文件。单通同时受 **1 小时与 512 MiB 解码样本**两项上限约束：最大秒数为 `min(3600, 512 × 1024² / (采样率 × 声道数 × 4))`，按完整采样帧向下取整。这是 float32 样本上限，不是 WAV 文件大小或进程总内存上限。

| 采样率 | 单声道最大时长 | 双声道最大时长 |
|---|---|---|
| 8／16 kHz | 60 分钟 | 60 分钟 |
| 32 kHz | 60 分钟 | 约 34.95 分钟 |
| 48 kHz | 约 46.60 分钟 | 约 23.30 分钟 |

真实单声道可分析，但不能独立验证双方时序，进入待复核。超过上限应先按事件分段或重采样；其他格式先用 `audio prepare` 转换并保留来源。读取同名 `.events.json` 和 `.provenance.json`；显式声道参数优先于文件映射，但冲突会拒绝，不能静默交换角色。缺少已核实的声道或接管时间不会自动通过。
...
2. **语音模型交叉检查**：固定 Silero VAD 6.2.2，独立处理两个声道，逐录音／逐声道重置模型状态。非 8/16 kHz 输入通过有抗混叠滤波的有理重采样处理。模型没有覆盖完整录音、输出无效或与声学证据冲突时进入复核。新应答至少有 0.25 秒模型语音和累计 0.25 秒重合的工程声学活动，计时从共同支持的起点开始；用户说完前已开始的输出不能当作新应答，只有重叠旧输出时转复核。
...
当前判定算法为 `qa-assessment-2`；本次审查修正后需重新生成旧报告，旧算法标签不能直接套用。漏答证据截至应答时限，迟答证据截至实际应答起点，试听窗口在两端各保留 1 秒上下文；其他独立异常仍保留各自窗口。
...
`qa model-doctor` 单独调用时未就绪仍退出 `1`；作为任务步骤时，未就绪或返回无效检查结果均记为 `failed`，依赖它的后续步骤跳过，独立步骤继续。正常 `qa assess` 的异常／待复核仍属于 `findings`，不会被混同为运行依赖失败。

## 波形辅助复核

0.16.2 起，整通报告恢复与逐轮复核页共用的双轨波形、时间轴和区间播放器。波形按原始幅度展示左右声道，只有映射已核实时才标注“用户／AI”；单声道显示“无法区分双方”，角色未知时明确提示未核实。

点击波形可定位，缩放／“当前范围”可放大疑点；青色边缘可拖动，也可用方向键调整试听起止。黄色区域保留当前建议复核范围。选择下方证据按钮会同步图上范围并开始试听；勾选循环则重复所选区间，取消循环后到终点暂停。单独试听左／右轨时，两轨图形仍在同一时间轴上。“试听音量”和“静音”控制声音，切换声道或录音后保留设置；显示增益仅放大图形，不改变音量或检测结果。

波形是每轨最多 1200 格的原始幅度摘要，适合看双方讲话、停顿和异常相对位置；放大不会增加摘要的时间分辨率，不能只凭图形证明回答语义正确。图形库和摘要随报告保存，不需联网加载。没有 `--include-audio` 时仍可看波形，播放需要音频副本；旧报告没有摘要时会显示明确提示并保留原有试听。

本次只补复核展示，`qa-assessment-2`、整通结果身份和 CSV 合同不变；升级页面无需重新执行模型，也不清空已有标签。任务结果包中的新摘要随报告重建，旧包仍可打开。人工结论仍为每通一条，不增加逐片段必填项。
