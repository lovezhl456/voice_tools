...
- 随后合入主线 `b46d681` 的 NISQA Docker 更新，保留 `0.11.1` 版本并核对 schema。Mac 全仓 **360 项，328 项执行通过、32 项可选测试跳过**，用时 60.533 秒。此次合并未改变运行代码或 ViSQOL 脚本；这份全仓结果是 Mac 回归，不是 Linux 全仓测试。
