...
“检索录音”支持文件名和路径片段的实时模糊匹配，不区分大小写；中文、号码片段、Windows 路径均可输入，多个关键词以空格分隔并同时匹配。检索与目录、录音、检测结果、证据及未标注筛选共同生效；清空检索恢复当前目录下的列表，不匹配的录音选择会清除，未记下的标注保留。检索不限制 CSV 导出范围；隐藏路径的页面仅按可见文件名检索。
...
复核页可先选目录，再选录音；目录按输入路径的直接父目录分组。默认“全部目录”显示原有列表；选定目录后的“全部录音”仅包含该目录。检测结果、证据和未标注筛选仍可组合。切换目录不会丢失未记下的表单，记下或放弃后再切换；无目录的文件归入“当前目录（.）”。`--hide-paths` 页面只保留文件名，不显示原目录。
