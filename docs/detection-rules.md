...
0.18.1 的[工作区](detection-workspace.md)在同一本机服务中提供持久化目录／日常规则组合、在线复核入库、未命中抽检、标准样本和固定集版本比较。下文静态报告和 CLI 合同继续保留。

## 图形配置界面

0.17.2 起可从报告的“定义指标与标签”入口编辑表单；需要直接保存版本和运行时使用 `detect serve` 本机服务。操作见[配置界面说明](detection-editor.md)。下列 JSON／CLI 流程和 1.0 数据格式继续兼容。
