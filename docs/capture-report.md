...
多主机、多通话的定时抓包、会话检索、FS 周期快照和 HOMER 联动见[批量会话指南](batch-sessions-homer.md)。本页保留单个 UUID 的操作说明。

`voice-tools capture` 通过 SSH 在指定 Linux 主机上查询活动 FreeSWITCH 通话，按端点运行 tcpdump，再用 SCP 取回 PCAP。`voice-tools report` 在本机汇总一份或多份录音、PCAP，输出离线 HTML 和 JSON。两者不需要 GPU 或在线模型。

## 1. 按 UUID 抓包

本机需要 OpenSSH 的 `ssh`、`scp`。远端需要 `fs_cli`、`tcpdump`、GNU `timeout`、`sha256sum` 和标准 shell 工具。SSH 用户须能调用本机 FreeSWITCH ESL API，并具备抓包权限；`fs_cli` 使用远端现有配置，不通过 CLI 传入 ESL 密码。`--fs-cli /usr/local/freeswitch/bin/fs_cli` 可指定安装路径。
...
```bash
# 只读查询：仍会 SSH 调用 fs_cli，但不会创建远端目录或启动 tcpdump
...
- `--seconds`：1–3600 秒，默认 60。远端 GNU timeout 给 tcpdump 发送 INT，5 秒后仍未退出则 KILL。SSH 断开后也有远端时限约束。
- `--snaplen`：64–65535，默认 2048。大于此长度的包会截断；报告统计截断包。需要完整大包时可设为 65535。
- `--max-mib`：1–512，默认 64。以 `floor((MiB×1048576−24)/(snaplen+16))` 推导 `tcpdump -c` 包数上限，限制经典 PCAP 的最坏大小。达到包数上限就提前结束，**不是滚动覆盖**。因此实际文件通常比额度小；snaplen 越大，同额度下允许的包数越少。
- `--sudo`：执行 `sudo -n timeout ... tcpdump ...`，要求已配置非交互权限。工具不配置 sudoers。tcpdump 用 `-Z` 切回当前 SSH 用户写入私有目录。
...
| 文件 | 内容 |
|---|---|
| `capture.json` | 端点、BPF、主机、UTC 时间、参数、远端路径、状态、SHA-256，以及可解析的 tcpdump 捕获/内核丢弃计数 |
| `session.pcap` | 校验通过的抓包文件 |
| `tcpdump.log` | tcpdump 标准错误，包括捕获/内核丢弃计数和运行错误 |
...
`fetch` 仅重试取回，不再查询 FS，也不能证明原 tcpdump 正常退出；恢复报告时使用 `--pcap ... --rtp-port ...`，或保留原清单供人工关联。Ctrl-C 中断本地命令时，远端可能继续至原定时限，清单保留恢复路径。
...
PCAP/PCAPNG 分析需要本机安装 Wireshark 的 `tshark`，可用 `--tshark /path/to/tshark` 指定。单份文件不超过 512 MiB，默认分析前 250000 包，`--max-packets` 最多 1000000；超过上限会标记部分结果并退出 3。
...
PT 0/8 的 RTP 时钟按 8000 Hz，PT 9（G.722）也按 RTP 规范使用 **8000 Hz**。动态 PT 默认未知，可重复 `--clock-rate PT=HZ` 指定；同一流有未知或变化的时钟时，不显示整体抖动值。映射需适合本批输入，不同抓包使用同一 PT 但不同编码时应分批分析。
...
报告不会自动进行录音与 PCAP 时间轴对齐、跨抓包点去重、SRTP 解密、RTP 语音解码、MOS 预测或根因判定。每份 PCAP 的缺口可能来自网络、抓包点丢弃、过滤和截断；`any` 接口上的重复也可能来自重复观测。单点抓包无法测单向网络延迟。音频低能量不能单独证明 AI/TTS 故障。

所有页面资源位于报告目录内，可离线打开，不请求 CDN。分享时须连同 `audio/` 和 `report.json` 一起复制；报告包含 IP、文件路径，启用试听时还包含录音。
...
2026-09-19 基于最新 `main` 的独立 PR 工作树执行 156 项测试，全部通过；其中抓包、会话检索、HOMER 联动及报告新增 41 项。使用真实 tshark、FFmpeg 和 tcpdump 做离线验证。桌面/手机渲染和实际试听尚未验收；本地浏览器此前因 URL 策略拒绝 `file://`，资源检查不等于播放成功。
