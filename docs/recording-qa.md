...
日常批量录音可使用 [整通自动质检](automatic-qa.md)：`qa assess` 以工程规则与本地模型联合判断，人工按整通处理例外与抽检。本文保留逐机会分析、细粒度标注和黄金集评估流程。
...
“检索录音”按文件名或路径片段实时匹配，不区分大小写，支持中文、号码片段和 Windows 路径；多个关键词用空格分隔，全部匹配才显示。例如 `客服 1380` 会查找路径含“客服”且含“1380”的录音。检索同时缩小录音下拉框和复核列表，并与目录及其他筛选取交集；清空检索可恢复当前目录内的列表。已选录音不匹配时自动恢复“全部录音”，未记下的标注继续保留。检索不会限制 CSV 导出范围；隐藏路径的页面只能检索保留下来的文件名。

`review.html` 可先选目录，再选该目录下的录音；目录按输入路径的直接父目录分组，不合并子目录。默认“全部目录”保留原有录音列表；选定目录后的“全部录音”仅包含该目录。检测结果、证据和未标注筛选可继续组合使用。切换目录会清除不在新目录内的录音选择，未记下的表单仍保留，记下或放弃后再切换。无目录的文件显示为“当前目录（.）”；使用 `--hide-paths` 生成的页面只能按隐藏后的文件名显示，不能恢复原目录。
