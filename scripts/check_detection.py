...
def main():
    ...
    try:
        ...
        expected={name:['D01','D02','D03','D04','D05'] for name in ('desktop','mobile')}
        result['capabilities']=coverage
        if coverage != expected:
            raise RuntimeError('D01–D05 desktop/mobile capability coverage is incomplete')
        if stats['expected'] != 10 or any(stats[key] for key in ('unexpected','skipped','flaky')):
            raise RuntimeError('Expected D01–D05 on desktop/mobile with no failures, skips or retries')
        result['status']='passed'
        print('Passed 10 installed detection browser cases: '+str(output))
        return 0
